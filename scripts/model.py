...
class DistRobustModel:
    def __init__(self,mu,cov,gamma1,gamma2,delta,k,timelimit=3600,a={0:1},b={0:0}):
        self.mu = mu
        self.cov = cov
        self.gamma1 = gamma1
        self.gamma2 = gamma2
    ...
    def write_sdpaformat(self,scale,wfname):
        ...
        with open(wfname, 'w') as wf:
            ...
            wf.write(str(self.num_stock+1)+' '+str(self.num_stock+1)+' '+str(-6*self.num_stock-3)+'\n')
            # set objective
            for index in dict_index_var:
                ...
                if var[0] == "x":
                    ...
                    j = int(var.split("_")[-1])
                    if i == j:
                        wf.write("{:.5f}".format(scale*self.gamma2*self.cov[i,j]-scale*self.mu[i]*self.mu[j])+" ")
                    else:
                        wf.write("{:.5f}".format(2*(scale*self.gamma2*self.cov[i,j]-scale*self.mu[i]*self.mu[j]))+" ")
                elif var[0] == "q":
                    i = int(var.split("_")[-1])
                    wf.write(str(0)+" ")
                elif var[0] == "r":
            ...
            block = 1
            for key in self.a:
                wf.write(str(0)+" "+str(block)+" "+str(self.num_stock+1)+" "+str(self.num_stock+1)+" "+"{:.16f}".format(-self.b[key])+"\n")
                for index in dict_index_var:
                    var = dict_index_var[index]
                    if "x" == var[0]:
                        i = int(var.split("_")[-1])
                        wf.write(str(index+1)+" "+str(block)+" " +str(i+1)+ " " + str(self.num_stock+1)+" "+"{:.16f}".format(0.5*self.a[key])+"\n")
                    elif "q" == var[0]:
                        i = int(var.split("_")[-1])
                        wf.write(str(index+1)+" "+str(block)+" " +str(i+1)+ " " + str(self.num_stock+1)+" "+"0.5\n")
                    elif "Q" == var[0]:
                        i = int(var.split("_")[-2])
                        j = int(var.split("_")[-1])
                        wf.write(str(index+1)+" "+str(block)+" " +str(i+1)+ " " + str(j+1)+" "+"1\n")
                    elif "r" == var[0]:
                        wf.write(str(index+1)+" "+str(block)+" " +str(self.num_stock+1)+ " " + str(self.num_stock+1)+" "+"1\n")
                block += 1
            ...
            for index in dict_index_var:
                ...
                if "p" == var[0]:
                    i = int(var.split("_")[-1])
                    wf.write(str(index+1)+" "+str(block)+" " +str(i+1)+ " " + str(self.num_stock+1)+" "+"1\n")
                elif "P" == var[0]:
                    i = int(var.split("_")[-2])
                    j = int(var.split("_")[-1])
                    wf.write(str(index+1)+" "+str(block)+" " +str(i+1)+ " " + str(j+1)+" "+"1\n")
            ...
            for i in range(self.num_stock):
                wf.write(str(0)+" "+str(block)+" "+str(i+1)+" " +str(i+1) + " -1\n")
            for index in dict_index_var:
                var = dict_index_var[index]
                if "x" == var[0]:
                    i = int(var.split("_")[-1])
            ...
            for i in range(self.num_stock):
                ...
                wf.write(str(index_p+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" 1\n")
                index_q = dict_var_index["q_"+str(i)]
                wf.write(str(index_q+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" 0.5\n")
                for j in range(self.num_stock):
                    if j < i:
                        index_Q = dict_var_index["Q_"+str(j)+"_"+str(i)]
                    else:
                        index_Q = dict_var_index["Q_"+str(i)+"_"+str(j)]
                    wf.write(str(index_Q+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" " + str(self.mu[j]) + "\n")
                constr_index += 1
            # -p -q/2 - Qmu >= 0
            for i in range(self.num_stock):
                index_p = dict_var_index["p_"+str(i)]
            ...
            for i in range(self.num_stock):
                ...
                wf.write(str(index_x+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" 1\n")
                constr_index+=1
            # z-x>=0
            for i in range(self.num_stock):
                index_x = dict_var_index["x_"+str(i)]
                ...
                wf.write(str(index_x+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" -1\n")
                wf.write(str(index_z+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" 1\n")
                constr_index+=1
            # -1^\topx <=-1
            wf.write(str(0)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" 1\n")
            for i in range(self.num_stock):
                index_x = dict_var_index["x_"+str(i)]
                wf.write(str(index_x+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" 1\n")
            constr_index+=1
            ...
            constr_index+=1
            # 1^\topz <=k
            wf.write(str(0)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" "+str(-self.k)+"\n")
            for i in range(self.num_stock):
                index_z = dict_var_index["z_"+str(i)]
                wf.write(str(index_z+1)+" "+str(block)+" "+str(constr_index+1)+" "+str(constr_index+1)+" -1\n")
            constr_index+=1
        ...
        def add_cutting_plane(model,where):
            if where == grb.GRB.callback.MIPSOL:
                ...
                if reduce_flag == 1:
                    ...
                    x_tmp,f,w_tmp,alpha_tmp,bata_tmp,lambda_tmp = self.solve_inner_dual(z=z_tmp)
                s = 1
                model.cbLazy(theta_var/s>= f/s + grb.quicksum(-(self.delta_/2)*(w_tmp[i]*w_tmp[i])*(z_var[i]-z_tmp[i]) for i in range(self.num_stock))/s)
    ...
    def solve_inner_primal(self,z):
        ...
        for key in self.a:
            M[key]=cp.Variable((self.num_stock+1,self.num_stock+1),symmetric=True)
        objective = cp.sum_squares(x)/(2*self.delta_) \
                   + cp.trace((self.gamma2*cov_sub-np.outer(mu_sub,mu_sub))@Q) \
                   + r \
                   + cp.trace(cov_sub @ P) \
                   - 2*(mu_sub.T @ p) + self.gamma1*s
        constraints = [p == - q/2- Q@mu_sub]
        constraints += [cp.sum(x) == 1, x>=0]
        constraints += [M2 >> 0]
        constraints += [M2[0:len(index),0:len(index)] == P,
                        M2[-1,0:len(index)] == p,
                        M2[len(index),len(index)]==s]
        for key in self.a:
            constraints += [M[key][0:len(index),0:len(index)] == Q,
    ...
    def solve_inner_dual(self,z):
        ...
        objective = -cp.sum_squares(w[index])*(self.delta_/2) -sum([self.b[key]*eta[key] for key in self.b])+pi
        constraints = [w >= sum([self.a[key]*beta[key] for key in beta]) + pi*np.ones(self.num_stock)]
        for i in range(self.num_stock):
            for j in range(self.num_stock):
            	constraints += [M3[i,j] == self.gamma2*self.cov[i,j]-self.mu[i]*self.mu[j] \
                                            - (alpha[i]*self.mu[j]+alpha[j]*self.mu[i])/2 - sum([B[key][i,j] for key in B])]
        constraints += [M3 == 0]
        ...
        for key in self.a:
            constraints += [M[key][0:self.num_stock,0:self.num_stock] == B[key],
                            M[key][self.num_stock,0:self.num_stock] == beta[key],
                            M[key][self.num_stock,self.num_stock]==eta[key]]
        constraints += [M2[0:self.num_stock,0:self.num_stock] == self.cov,
    ...
    def solve_inner_reduced_dual(self,z):
        ...
        objective = -cp.sum_squares(w)*(self.delta_/2) -sum([self.b[key]*eta[key] for key in self.b]) +pi
        constraints = [w >= sum([self.a[key]*beta[key] for key in self.a]) + pi*np.ones(len(index))]
        scale = 1
        for i in range(len(index)):
            for j in range(len(index)):
                constraints += [M3[i,j]/scale == self.gamma2*cov_sub[i,j]/scale-mu_sub[i]*mu_sub[j]/scale \
                                            - (alpha[i]*mu_sub[j]+alpha[j]*mu_sub[i])/2/scale - sum([B[key][i,j] for key in B])/scale]
        constraints += [M3 == 0]
        constraints += [-2*mu_sub-alpha-2*lambda_ == 0]
        constraints += [-alpha/2 - sum([beta[key] for key in beta])  == 0]
        constraints += [sum([eta[key] for key in eta])==1]
        for key in self.a:
            constraints += [M[key][0:len(index),0:len(index)] == B[key],
                            M[key][len(index),0:len(index)] == beta[key],
                            M[key][len(index),len(index)]==eta[key]]
        constraints += [M2[0:len(index),0:len(index)] == cov_sub,
        ...
        lambda_full = self.complete_lambda(Sigma=self.gamma1*self.cov,lambda_=lambda_opt,index=index)
        beta_full = {}
        for key in beta:
            beta_full[key] = self.complete_beta(Sigma=self.gamma2*self.cov,beta=beta[key].value,eta=eta[key].value,index=index)
        ...
        for i in range(self.num_stock):
            w_full_dic[i] = np.max([sum([self.a[key]*beta_full[key][i] for key in self.a])+pi_opt,0])
            if i not in index:
                x_dic[i] = 0
...
class MeanVarianceModel:
    ...
    def solve(self):
        ...
        model.addConstr(grb.quicksum(self.mu[i]*x[i] for i in range(self.num_stock))>=self.r_bar)
        model.addConstr(grb.quicksum(z[i] for i in range(self.num_stock)) <= self.k)
        model.setObjective(grb.quicksum(self.cov[i,j]*x[i]*x[j] for j in range(self.num_stock) for i in range(self.num_stock)))
        model.optimize()
...
class RobustModel:
    ...
    def solve(self):
        ...
        for i in range(self.num_stock):
            x[i] = model.addVar(vtype='C',name='x_'+str(i),ub=1)
            z[i] = model.addVar(vtype='B',name='z_'+str(i),ub=1)
        v = model.addMVar(self.num_stock,vtype='C', lb=-grb.GRB.INFINITY, ub=grb.GRB.INFINITY)
        aux = model.addVar(vtype='C',lb=0)
        model.update()
        for i in range(self.num_stock):
            model.addConstr(x[i]<=z[i])
            model.addConstr(v[i] == grb.quicksum(self.L[j,i]*x[i] for j in range(self.num_stock)))
        model.addConstr(grb.quicksum(x[i] for i in range(self.num_stock)) == 1)
        model.addConstr(aux*aux >= self.kappa*grb.quicksum(v[i]*v[i] for i in range(self.num_stock) ))
        model.addConstr(grb.quicksum(z[i] for i in range(self.num_stock)) <= self.k)
        model.setObjective(grb.quicksum(self.cov[i,j]*x[i]*x[j] for j in range(self.num_stock) for i in range(self.num_stock)) 
                            - self.sigma*grb.quicksum(self.mu[i]*x[i] for i in range(self.num_stock))
                            +self.sigma*aux)
...
class BNTRobustModel:
    ...
    def solve(self):
        ...
        for i in range(self.num_stock):
            ...
            model.addConstr(v[i] == grb.quicksum(self.L[j,i]*x[i] for j in range(self.num_stock)))
        model.addConstr(grb.quicksum(x[i] for i in range(self.num_stock)) == 1)
        model.addConstr(aux*aux >= grb.quicksum(v[i]*v[i] for i in range(self.num_stock) ))
        model.addConstr(grb.quicksum(z[i] for i in range(self.num_stock)) <= self.k)
        model.setObjective( - grb.quicksum(self.mu[i]*x[i] for i in range(self.num_stock))+np.sqrt(self.kappa)*aux)
        model.params.NumericFocus = 3
        model.optimize()
        if model.Status == grb.GRB.OPTIMAL:
            z_opt = {}
